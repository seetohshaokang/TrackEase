...
function EventList({ events, setEvents }) {
  ...
  const filteredEvents = sortedEvents.filter((event) => {
    const endDateTime = new Date(event.end.dateTime || event.end.date).getTime();
  ...
  return (
    <div className="w-full flex flex-col">
        ...
        <div className="flex flex-row pb-2"> 
          <button
            className={`py-0 px-4 rounded-md text-xl ${activeTab === "upcoming" ? "bg-gray-300" : "bg-gray-100"}`}
            onClick={() => setActiveTab("upcoming")}
          >
            Upcoming
          </button>
          <button
            className={`py-2 px-4 rounded-md text-xl ${activeTab === "past" ? "bg-gray-300" : "bg-gray-100"}`}
            onClick={() => setActiveTab("past")}
          >
            Past
          </button>
        <button
  className="ml-auto py-2 px-4 rounded-lg bg-gray-100 border-2 flex items-center tooltip tooltip-top" data-tip={sortOrder === "asc" ? "Sort by Descending" : "Sort by Ascending"}
      ...
      {filteredEvents.length === 0 ? (
        ...
        <ul>
          {filteredEvents.map((event) => {
            ...
            return (
              <div className="py-2">
              <li key={event.id} className="flex justify-between items-center p-2 shadow-md rounded-md bg-gray-100">
                <div>
                  ...
                  <div className="border-2 rounded-lg p-2 tooltip tooltip-right" data-tip="View in Google Calendar">
                    <Link
                      href={event.htmlLink}
                      target="_blank"
                ...
                <button onClick={() => handleDelete(event.id)} className="btn bg-red-600 text-white btn-sm mr-5" title="Delete">
